...
Implements the internal "Search / Query as a Generated, Grounded Artifact" pattern
...
The four disciplines (see internal SEARCH_AS_CODE_PATTERN.md): ground the LLM in a
...
def compile_nl_to_plan(
    ...
    Exactly one LLM call. ``llm_fn`` defaults to a local Ollama generate (internal
